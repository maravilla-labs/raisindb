...
#else
    // RAISINDB PATCH: -1, not 0. With a zero default, close() after a failed open
    // (and every second close) called ::close(0), closing whatever the process
    // held on fd 0 at that moment -- in raisindb, the RocksDB WAL.
    int file_descriptor_ = -1; /**< The file descriptor on Linux and MacOS. */
#endif
...
#else
          file_descriptor_(exchange(other.file_descriptor_, -1))
#endif
    {
    }
...
#else
        // RAISINDB PATCH: only release what open_if_not() actually acquired.
        if (ptr_)
            munmap(ptr_, length_);
