 ...
 * TODO: Re-integrate once the new SDK-based AI chat widget is implemented.
 */
function openAIChatWithContext() {
  if (!browser) return;
  // AI chat store was removed during Phase 4 frontend consolidation.
  // Voice activation detects the wake word but cannot open AI chat yet.
  console.log('[Voice] Wake word detected. AI chat integration pending.');
}
...
async function sendCommandToAI(command: string) {
  ...
  console.log('[Voice] Command captured (AI chat integration pending):', command);
}
