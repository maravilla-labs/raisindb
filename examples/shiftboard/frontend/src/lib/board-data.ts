/**
 * Pure data mapping for the shift board — shared between the SSR load
 * (+page.server.ts, SQL over HTTP) and the client rune store
 * (stores/board.svelte.ts, SQL + live events over WebSocket).
 *
 * Keeping this free of runes / browser APIs makes it safe on both sides.
 */
...
/** SQL used by both the SSR load and the client resync after reconnect. */
