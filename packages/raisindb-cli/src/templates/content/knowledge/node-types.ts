export function nodeTypesKnowledge(): string {
  return `# RaisinDB Node Types Reference
...
\`\`\`
Mixin (shared props) → NodeType (schema) → Archetype (page template) → ElementTypes (blocks)
                            ↓                      ↓                         ↓
...
Mixins are NodeTypes marked with \`is_mixin: true\`. They define a reusable set of
properties that can be composed into multiple NodeTypes, avoiding property duplication.
...
### Using Mixins in NodeTypes

NodeTypes can include mixin properties by referencing the mixin. This composes the
mixin's properties into the NodeType at install time.

### Package Structure

Mixins live in the \`mixins/\` directory of a package and are listed under
\`provides.mixins\` in \`manifest.yaml\`. During package installation, mixins are
installed BEFORE NodeTypes since NodeTypes may reference them.
...
- Mixins: \`package/mixins/<name>.yaml\`
