 ...
 *
 * Displays the list of properties in the nodetype.
          ...
          {nodeType.properties.length === 0 ? (
            ...
            `}>
              ...
              <p className="text-zinc-400 text-sm font-medium">
                {isDraggedOver ? 'Drop here to add property' : 'No properties yet'}
              </p>
              <p className="text-zinc-600 text-xs mt-1">
                Drag property types from the left toolbox
              </p>
            </div>
          ) : (
            <>
              {nodeType.properties.map((property, index) => (
                <PropertyItem
                  key={property.id}
                  property={property}
                  index={index}
                  path={property.name || `prop_${index}`}
