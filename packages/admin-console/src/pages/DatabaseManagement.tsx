...
import { Database, Search, Sparkles, AlertCircle, RefreshCw, Zap, Trash2, CheckCircle, XCircle, Clock, Activity, Loader2, ShieldAlert, Wrench } from 'lucide-react'
...
export default function DatabaseManagement() {
  ...
  // v0.1.29: per-(tenant, repo, branch) fulltext error counter. `null`
  // means we haven't loaded yet; a stats object with `total === 0`
  // means "all clear". Counters are in-memory on the server and reset
  // across restarts, so a fresh-process state also shows as 0.
  ...
  // Load fulltext health AND error counter when repo/branch changes.
  // We do both here so the "Index Errors" card always reflects the
  // current selection without an extra trigger.
  useEffect(() => {
    if (selectedRepo) {
      loadFulltextHealth()
      loadFulltextErrors()
    }
  }, [selectedRepo, tenant, branch])
  ...
  async function loadFulltextErrors() {
    if (!selectedRepo) return
    try {
      const stats = await databaseManagementApi.fulltextErrors(tenant, selectedRepo, branch)
      setFulltextErrors(stats)
    } catch (error: any) {
      // The error endpoint should never fail except on transport
    ...
    openConfirmDialog({
      ...
      action: async () => {
        ...
        try {
          ...
          // Rebuild clears the on-disk index and re-indexes from
          // RocksDB. Past failures are no longer meaningful — clear
          // the counter optimistically so the dashboard goes green.
          // If new errors come in during the rebuild itself they'll
          // repopulate on the next poll.
   ...
   * against the existing Tantivy directory so any entries dropped by
   * the v0.1.28 aggregator bug get added back. Safe on healthy
   * indexes (just wasted I/O).
   */
  async function reconcileFulltext() {
    if (!selectedRepo) return
    setOperationLoading('reconcile')
    try {
      const response = await databaseManagementApi.fulltextReconcile(tenant, selectedRepo, branch)
  ...
  async function clearFulltextErrorsSilent() {
    if (!selectedRepo) return
    try {
      await databaseManagementApi.fulltextErrorsClear(tenant, selectedRepo, branch)
      setFulltextErrors(null)
    } catch (error: any) {
      // Counter clear is best-effort. If it fails, the next page
      // load (or auto-poll) will resync.
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-red-300/80">
                    Counts since server start for {tenant}/{selectedRepo}/{branch}. Run Reconcile to re-index from the canonical node store, then clear.
                  </p>
                </div>
              </div>
              <button
