...
fn test_method_call() {
    ...
    if let Expr::MethodCall { method, args, object } = e {
        assert_eq!(method, "contains");
        assert_eq!(args.len(), 1);
...
fn test_relates_simple() {
    ...
    } = e
    {
        ...
        assert_eq!(direction, crate::ast::RelDirection::Any);
    } else {
...
fn test_relates_with_direction() {
    ...
    } = e
    {
        ...
        assert_eq!(min_depth, 1);
        assert_eq!(max_depth, 1);
        assert_eq!(direction, crate::ast::RelDirection::Outgoing);
    } else {
        panic!("Expected Relates, got {:?}", e);
    }
}
...
fn test_relates_full() {
    ...
    } = e
    {
        ...
        assert_eq!(direction, crate::ast::RelDirection::Incoming);
    } else {
        panic!("Expected Relates, got {:?}", e);
    }
}
...
fn test_relates_in_boolean_expr() {
    let e = parse("input.value > 10 && node.created_by RELATES auth.local_user_id VIA 'FRIENDS_WITH'");
    if let Expr::BinaryOp { op, left, right } = e {
        assert_eq!(op, BinOp::And);
        assert!(matches!(*left, Expr::BinaryOp { op: BinOp::Gt, .. }));
