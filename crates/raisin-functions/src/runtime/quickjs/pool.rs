...
//! 7. **A reused runtime is vetted against the incoming memory limit.** See
//!    [`QuickJsPool::checkout`]; without this, invariant 4 is actively harmful.
...
/// The share of a function's memory budget a *reused* runtime is allowed to
/// have already consumed before checkout rejects it.
///
/// A runtime that arrives half-full leaves the function half its declared
/// budget, so the worst case is bounded at 2× rather than "works, or fails at
/// the first allocation". A warm runtime with no live context sits in the low
/// hundreds of kilobytes, far under any real limit, so ordinary reuse is
/// unaffected — only genuinely bloated runtimes are discarded.
const MAX_INHERITED_MEMORY_SHARE: f64 = 0.5;
...
impl QuickJsPool {
    ...
    /// Check out a runtime for the exclusive use of one execution that will run
    /// under `memory_limit` bytes.
    ///
    ...
    ///
    /// # Why the limit is an argument
    ///
    /// `set_memory_limit` is *runtime*-global (invariant 1) but the limit
    /// itself is **per function**, and a pooled runtime carries whatever the
    /// previous occupant left behind. So a modest function could be handed a
    /// runtime another function had already grown past that function's own
    /// limit, and QuickJS would fail its very first allocation — surfacing as
    /// `Failed to setup JS environment: out of memory`, or, when it left the
    /// interpreter in an exception state with no exception object, as the bare
    /// `Exception generated by QuickJS`. Intermittent by construction: it
    /// depended entirely on which runtime the pool happened to hand out, which
    /// is exactly how it read in production — one busy mount churning the pool,
    /// a quiet one failing "transiently" beside it.
    ///
    /// Nothing ran a GC between executions either. QuickJS reclaims cycles only
    /// on `run_gc`, and a fresh full context per execution makes cycles the
    /// normal case, so a runtime accumulated for up to
    /// [`DEFAULT_RECYCLE_THRESHOLD`] executions. Collecting here is what makes
    /// the measurement below mean "genuinely retained" rather than "not yet
    /// collected", so the two halves have to stay together.
    pub(super) async fn checkout(&self, memory_limit: usize) -> Result<Checkout<'_>> {
        let permit = self
            .permits
            .acquire()
            .await
        ...
        // Take idle runtimes until one can host this function, discarding those
        // that cannot. Popping rather than scanning keeps the idle lock
        ...
        loop {
            ...
            candidate.runtime.run_gc().await;
            let retained = candidate.runtime.memory_usage().await.malloc_size.max(0) as usize;

            if (retained as f64) <= memory_limit as f64 * MAX_INHERITED_MEMORY_SHARE {
            ...
            tracing::debug!(
                ...
                memory_limit,
                uses = candidate.uses,
                "discarding a pooled QuickJS runtime that cannot host this function's memory limit"
            );
            drop(candidate);
        }
