...
//! Signed URL handlers for asset access in RaisinDB.
//!
//! This module provides handlers for generating and verifying signed URLs
//! for secure asset access (download and display commands).
...
pub fn parse_asset_command_from_path(path: &str) -> Option<(String, String)> {
...
pub fn parse_sign_command_from_path(path: &str) -> Option<String> {
...
///
/// This function handles /raisin:download and /raisin:display commands
...
pub async fn handle_asset_command_internal(
    ...
    let node = state
        .storage()
        .nodes()
        .get_by_path(tenant_id, repo, branch, ws, &node_path, None)
        .await?
    ...
    let resource = match file_prop {
        ...
        _ => {
            return Err(ApiError::not_found(
                format!("Node's '{}' property is not a Resource type", prop_name),
            ));
        }
    };
...
///
/// This function handles /raisin:sign commands by generating a signed URL
/// for accessing the specified asset.
pub async fn sign_asset_url_internal(
    ...
    match file_prop {
        ...
        _ => {
            return Err(ApiError::validation_failed(
                format!("Node's '{}' property is not a Resource type", property_path),
            ));
        }
    }
    ...
    let full_path = if property_path == "file" {
        ...
        format!("{}/{}/head/{}{}@{}", repo, branch, ws, node_path, property_path)
    };
...
// Note: Asset command handling (raisin:download, raisin:display, raisin:sign)
// is integrated into repo_get and repo_post handlers using the internal functions:
// - parse_asset_command_from_path() - detects download/display in path
// - parse_sign_command_from_path() - detects sign in path
// - handle_asset_command_internal() - serves binary with signed URL validation
// - sign_asset_url_internal() - generates signed URLs
