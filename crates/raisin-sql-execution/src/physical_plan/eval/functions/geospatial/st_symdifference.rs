//! ST_SYMDIFFERENCE function - compute the symmetric difference of two geometries
    ...
    fn evaluate(&self, args: &[TypedExpr], row: &Row) -> Result<Literal, Error> {
        if args.len() != 2 {
            return Err(Error::Validation(
                "ST_SYMDIFFERENCE requires exactly 2 arguments".to_string(),
            ));
        }
        ...
        let geom1 = match &geom1_val {
            ...
            _ => {
                return Err(Error::Validation(
                    "ST_SYMDIFFERENCE requires GEOMETRY arguments".to_string(),
                ))
            }
        };
        ...
        match (type1, type2) {
            ...
            _ => Err(Error::Validation(format!(
                "ST_SYMDIFFERENCE not supported for {} and {}. Supports: Polygon+Polygon",
                type1, type2
            ))),
        }
    }
