...
impl TantivyIndexingEngine {
    ...
    pub fn do_batch_index(
        ...
        let index_key = Self::index_key(&context.tenant_id, &context.repo_id, &context.branch);

        let processed = self.with_writer(&index_key, index, |writer| {
            let mut processed = 0;
                ...
                if let Some(translations) = &node.translations {
                    for lang_code in &context.supported_languages {
                        ...
                        writer.add_document(doc).map_err(|e| {
                            Error::storage(format!("Failed to add document: {}", e))
                        })?;
                    }
                }
