...
/// What an adapter says it can do, returned by its `capabilities` operation.
///
/// Declared ONCE, in `raisin-models`, and re-exported here. It used to be
/// declared twice — this engine and the HTTP connection-test handler each had
/// a copy, both serializing into the ONE `capabilities` property on the
/// `raisin:Integration` node — and the copies had drifted by three fields, so
/// clicking "Test connection" silently stripped them from the cached blob and
/// the console could never show `submit_unavailable_reason`. The module docs in
/// `raisin_models::nodes::integrations` carry the full account.
